...
vec3 camCartesianWorldPos{3, 0, -1};
...
Sphere spheres[] = {
    ...
    Sphere{vec3{8, -4, 0}, 5, Color{0, 255, 255, 255}},
    Sphere{vec3{3, 2, 1}, 0.5, Color{255, 255, 255, 255}},
    Sphere{vec3{5, -8, 4}, 0.25, Color{255, 255, 0, 255}},
    Sphere{vec3{5, 2, -1}, 0.25, Color{0, 255, 0, 255}},
    Sphere{vec3{-10, 0, 0}, 7, Color{0, 0, 255, 255}},
};
...
int main() {
  ...
  SDL_Window *window =
      SDL_CreateWindow("Example: 0", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, 0);
  if (!window) {
      ...
      for (int y = 0; y < SCREEN_HEIGHT; y++) {
        if (!rays[x][y].didHit) {
          ...
          double magnitude = std::sqrt(
                        rays[x][y].state.y2 * rays[x][y].state.y2 +
