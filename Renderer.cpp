  ...
  for (int x = 0; x < SCREEN_WIDTH; x++) {
    for (int y = 0; y < SCREEN_HEIGHT; y++) {
      ...
      Matrix4x4 RotationMatrix = Matrix4x4{
          ...
          ToVec4(Normalize(Cross(rightVector, camCartesianWorldDirection)),
                 camCartesianWorldPos.y),
          ToVec4(camCartesianWorldDirection, camCartesianWorldPos.z),
          vec4{0, 0, 0, 1}};
      vec3 pointInWorldSpace =
          ToVec3(Inverse(RotationMatrix) *
                 ToVec4(pointInCamSpace - camCartesianWorldPos, 0));
      ...
      vec3 polarVelocity =
          SphericalTransformationAt(camPolarPos, rayDirection) * c;

      // Apply
      vec8 startingState = vec8{
          camPolarPos.x, polarVelocity.x, 0, 0, camPolarPos.y, polarVelocity.y,
      ...
      for (int y = 0; y < SCREEN_HEIGHT; y++) {
        if (!rays[x][y].didHit) {
          ...
          for (int i = 0; i < sphereCount; i++) {
            vec3 cartPos = CartesianTransformaion(vec3{
                rays[x][y].state.y1, rays[x][y].state.y5, rays[x][y].state.y7});
            vec3 relativeToSphere = cartPos - spheres[i].cartPos;
            double r = std::sqrt(relativeToSphere.x * relativeToSphere.x +
                                 relativeToSphere.y * relativeToSphere.y +
                                 relativeToSphere.z * relativeToSphere.z);
          ...
          rays[x][y].state = rays[x][y].state +
                             (k1 + (k2 * 2) + (k3 * 2) + k4) * (timeStep / 6);

          // Normalisation
          double magnitude =
              std::sqrt(rays[x][y].state.y2 * rays[x][y].state.y2 +
                        rays[x][y].state.y4 * rays[x][y].state.y4 +
                        rays[x][y].state.y6 * rays[x][y].state.y6 +
                        rays[x][y].state.y8 * rays[x][y].state.y8);
