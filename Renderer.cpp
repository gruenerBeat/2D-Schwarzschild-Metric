...
vec3 camCartesianWorldPos{5, 0, 0};
vec3 camCartesianWorldDirection{-1, 0, 0};
constexpr double f = 20;

Sphere spheres[] = {
    Sphere{vec3{0, 0, 0}, 2, Color{255, 0, 0, 255}},
    Sphere{vec3{-1, 1, 1}, 2, Color{255, 127, 0, 255}},
};
...
int main() {
  ...
  for (int x = 0; x < SCREEN_WIDTH; x++) {
    for (int y = 0; y < SCREEN_HEIGHT; y++) {
      ...
      Matrix3x3 InverseIntrinsicMatrix =
          !Matrix3x3{vec3{f, 0, 0}, vec3{0, f, 0},
                     vec3{SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 1}};
      vec3 screenCoords = vec3{x, y, 1};
      vec3 pointInCamSpace = InverseIntrinsicMatrix * screenCoords;
      vec3 rightVector =
          Normalize(Cross(vec3{0, 1, 0}, camCartesianWorldDirection));
      Matrix3x3 RotationMatrix =
          Matrix3x3{rightVector,
                    Normalize(Cross(camCartesianWorldDirection, rightVector)),
                    Normalize(camCartesianWorldDirection)};
      vec3 pointInWorldSpace =
          !(~RotationMatrix) * (pointInCamSpace - camCartesianWorldPos);
      vec3 rayDirection = Normalize(pointInWorldSpace - camCartesianWorldPos);

      // Check for Collision
      for (int i = 0; i < sphereCount; i++) {
        spheres[i].cartPos = vec3{spheres[i].cartPos.x, spheres[i].cartPos.y,
                                  spheres[i].cartPos.z};
        double dst;
        ...
        double c = Dot(relativeRayPos, relativeRayPos) -
      ...
      SDL_SetRenderDrawColor(renderer, rays[x][y].hitColor.r,
                             rays[x][y].hitColor.g, rays[x][y].hitColor.b,
                             rays[x][y].hitColor.a);
    ...
    for(int x = 0; x < SCREEN_WIDTH; x++) {
      for(int y = 0; y < SCREEN_HEIGHT; y++) {
        if(!rays[x][y].didHit) {
          ...
          for(int i = 0; i < sphereCount; i++) {
            //vec3 sphereRelative =
  PolarTransformation(CartesianTransformaion(vec3{rays[x][y].state.y1,
  rays[x][y].state.y5, rays[x][y].state.y7}) + (spheres[i].cartPos * -1));
            //double r = std::sqrt(rays[x][y].state.y1 * rays[x][y].state.y1 +
  rays[x][y].state.y5 * rays[x][y].state.y5 + rays[x][y].state.y7 *
  rays[x][y].state.y7); if(rays[x][y].state.y1 <= spheres[i].radius) { hit =
  true; rays[x][y].hitColor = spheres[i].color;
            }
          }
          if(hit) {
            rays[x][y].didHit = true;
          ...
          rays[x][y].state = rays[x][y].state + (k1 + (k2 * 2) + (k3 * 2) + k4)
  * (timeStep / 6);

          //Normalisation
          double magnitude = std::sqrt(rays[x][y].state.y2 * rays[x][y].state.y2
  + rays[x][y].state.y4 * rays[x][y].state.y4 + rays[x][y].state.y6 *
  rays[x][y].state.y6 + rays[x][y].state.y8 * rays[x][y].state.y8);
          ...
          if(x == SCREEN_WIDTH / 2 && y == SCREEN_HEIGHT / 2) {
            std::cout << rays[x][y].state.y1 << " " << rays[x][y].state.y5 << "
  " << rays[x][y].state.y7 << "\n";
          }
